...
実際のプロジェクトに入る前に、体験用リポジトリ `nonpro-sandbox` を使って、チーム開発の一連の流れ（Git/GitHubの基本操作）をCLI（ターミナル）上で練習してみましょう。参加者自身が自ら課題（イシュー）を立てて解決する流れを体験します。

### [0] Sandboxリポジトリへの招待を受ける
開発に参加するためには、まず管理者からリポジトリへのアクセス権限（Write権限）をもらう必要があります。
1. Slack等で管理者（タカハシさん）にGitHubのアカウント名（例: `miyakoda-system`）を伝え、`nonpro-sandbox` への招待を依頼してください。
2. GitHubから招待メールが届く、または [GitHubのNotifications](https://github.com/notifications) に招待が届くので、**"Accept invitation"** をクリックして参加を承認します。

### [1] リポジトリのクローン
まずは練習用の sandbox リポジトリをダウンロード（クローン）します。
ターミナルで作業フォルダ（例: `nonpro-projects`）を開き、AIに以下のように依頼します。

> **依頼プロンプト**:
> 「GitHubから `plannauts/nonpro-sandbox` のリポジトリをクローンして」
